...
{
	/*All expected angles return a ratio of 1/sqrt(2).
	Sqrt(2) cancels this to give the circumscribed square size.*/
	for (int i = 0; i < 4; i++)
	{
		block[objectNumber].vertices[i].xPosition = block[objectNumber].centre.xPosition + 
			((block[objectNumber].dimensions.width / 2)	* sqrt(2) * (cos((45 + (i * 90)) * (PI / 180))));
		block[objectNumber].vertices[i].yPosition = block[objectNumber].centre.yPosition + 
			((block[objectNumber].dimensions.height / 2) * sqrt(2) * (sin((45 + (i * 90)) * (PI / 180))));
	}
}

void geometry_plotPolygon(int objectNumber)
{
	/* Polygon defined sides divide a full circle into angle
	increments that are multiplied by i to plot all points.*/
	for (int i = 0; i < polygon[objectNumber].properties.edges; i++)
	{
...
void geometry_transform()
{
	...
	for (int i = 0; i <= storedPolygons; i++)
	{
			...
			&& polygon[i].properties.classification != PLACEHOLDER)
		{
			...
			for (int j = 0; j < polygon[i].properties.edges; j++)
			{
	...
	for (int i = 0; i <= storedPolygons; i++)
	{
		if (polygon[i].properties.classification != NOTHING
			&& polygon[i].properties.classification != PLACEHOLDER)
		{
			/*Prevents the angle displacement values from exceeding the 0 to 360 range*/
